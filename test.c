...
#include "basic_signal_math/comparator_test.hpp"
...
// Wrappers
PointType wrapper_step(void* g) { return ((StepGenerator*)g)->generate(); }
PointType wrapper_ramp(void* g) { return ((RampGenerator*)g)->generate(); }
PointType wrapper_delta(void* g) { return ((DeltaGenerator*)g)->generate(); }
PointType wrapper_exp(void* g) { return ((ExponentialGenerator*)g)->generate(); }
PointType wrapper_saw(void* g) { return ((SawtoothGenerator*)g)->generate(); }
PointType wrapper_tri(void* g) { return ((TriangleGenerator*)g)->generate(); }

int main() {
    const int no_of_samples = 41;

    StepGenerator stepGen(1.0f, 0.0f, 1.0f);
    RampGenerator rampGen(2.0f, 1.0f);
    DeltaGenerator deltaGen(5.0f, 0.0f, 1.0f);
    ExponentialGenerator expGen(1.0f, 1.0f, 1.0f);
    SawtoothGenerator sawGen(1.0f, 20.0f, 1.0f);
    TriangleGenerator triGen(1.0f, 40.0f, 1.0f);

    float times[no_of_samples];
    for (int i = 0; i < no_of_samples; ++i) times[i] = static_cast<float>(i);
    ...
    comparator_test();  // Assuming this is still available and working
